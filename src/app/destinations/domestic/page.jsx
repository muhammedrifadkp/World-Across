...
import { MapPin, Star, Calendar, Clock, ArrowRight, Filter, Search, Heart, Camera } from 'lucide-react';
import { destinationAPI } from '@/lib/staticApi';
import ClientOnly from '@/components/ClientOnly';
import Link from 'next/link';

const DomesticDestinationsPage = () => {
  ...
  useEffect(() => {
    const fetchDestinations = async () => {
      try {
        ...
        const domesticDestinations = response.data.destinations.filter(dest => 
          dest.types.includes('Domestic')
        );
        setDestinations(domesticDestinations);
        setFilteredDestinations(domesticDestinations);
      } catch (error) {
  ...
  const highlights = [
    {
      ...
      description: 'Discover the diverse beauty of India from pristine beaches to majestic mountains',
    ...
    },
    {
      icon: MapPin,
      title: 'Easy Travel',
      description: 'No visa hassles, familiar culture, and convenient domestic flights',
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Explore the incredible diversity of India. From the beaches of Goa to the mountains of Himachal, 
            discover amazing destinations right in your backyard.
          </p>
        </motion.div>
        ...
        >
          <p className="text-gray-600">
            Showing {filteredDestinations.length} domestic destination{filteredDestinations.length !== 1 ? 's' : ''}
                ...
                <div className="p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="text-center">
                      <div className="text-lg font-bold text-orange-600">{destination.packageCount}</div>
                  ...
                  <Link href={`/destinations/${destination.id}`}>
                    <button className="w-full bg-gradient-to-r from-orange-600 to-red-600 text-white py-3 rounded-xl font-bold hover:shadow-lg transition-all duration-300 flex items-center justify-center space-x-2 group">
        ...
        {filteredDestinations.length === 0 && !loading && (
          ...
          >
            ...
            <button
              ...
              className="bg-orange-600 text-white px-6 py-3 rounded-xl font-medium hover:bg-orange-700 transition-colors duration-200"
            >
              Clear Filters
            </button>
          </motion.div>
        ...
        <motion.div
          ...
          className="mt-20 bg-gradient-to-r from-orange-600 to-red-600 rounded-3xl p-12 text-center text-white"
        >
          <h2 className="text-3xl md:text-4xl font-bold mb-4">
            Ready to Explore India?
          </h2>
          <p className="text-xl mb-8 opacity-90">
            Discover the incredible diversity of domestic destinations with our curated travel packages
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/packages?type=domestic">
              <button className="bg-white text-orange-600 px-8 py-4 rounded-xl font-bold hover:bg-gray-100 transition-colors duration-200">
                View Domestic Packages
              </button>
            </Link>
            <Link href="/contact">
              <button className="border-2 border-white text-white px-8 py-4 rounded-xl font-bold hover:bg-white hover:text-orange-600 transition-all duration-200">
                Plan Custom Trip
              </button>
            </Link>
          </div>
...
export default DomesticDestinationsPage;
